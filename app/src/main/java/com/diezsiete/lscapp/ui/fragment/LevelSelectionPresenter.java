...
public class LevelSelectionPresenter<V extends LevelSelectionContract.MvpView> extends BasePresenter<V>
        implements LevelSelectionContract.Presenter<V> {

    @Inject
    public LevelSelectionPresenter(DataManager dataManager) {
        super(dataManager);
    }
    ...
    public void onLevelClicked(Level level) {
        getDataManager().setCurrentLevel(level);
        getMvpView().openLevelActivity();
    }
}
